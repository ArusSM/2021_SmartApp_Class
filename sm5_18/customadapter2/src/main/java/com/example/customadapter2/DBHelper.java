...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String msgTable="create table msgTable ("+
                "_id integer primary key autoincrement," +
                "title," +
                "date," +
                "type)";
        db.execSQL(msgTable);
        db.execSQL("insert into msgTable (title, date, type) values ('홍길동','19.05.18', '오늘은 무슨날?')");
        db.execSQL("insert into msgTable (title, date, type) values ('야길동','19.05.18', '오늘은 무슨날?')");
        db.execSQL("insert into msgTable (title, date, type) values ('코길동','19.05.18', '오늘은 무슨날?')");
        db.execSQL("insert into msgTable (title, date, type) values ('야코동','19.05.18', '오늘은 무슨날?')");
        db.execSQL("insert into msgTable (title, date, type) values ('오동동','19.05.18', '오늘은 무슨날?')");
        db.execSQL("insert into msgTable (title, date, type) values ('우동','19.05.18', '오늘은 무슨날?')");
